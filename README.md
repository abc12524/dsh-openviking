...
**工具可见性**：九个工具在插件加载时**无条件注册**，因此始终出现在工具列表中（harness 在 agent 构造时从全局工具表快照，迟到的动态注册/注销无法可靠生效）。若尚未在设置页配置 `url`/`key`，调用任意工具会由 `safe()` 包装返回清晰的 `{"error":"OpenViking 未配置…"}` 提示，而非把工具藏起来；配好地址与密钥后即可正常调用，无需重启。
