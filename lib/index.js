...
*/
function decodeUserFromKey(key) {
	const segment = key.split(".")[1];
	if (segment === void 0 || segment.length === 0) return "default";
	try {
...
var OpenVikingRestClient = class {
	...
	*/
	async remember(category, name, content, signal) {
		...
		const uri = `viking://user/${decodeUserFromKey(key)}/memories/${category}/${name}.md`;
		try {
	...
	async readFile(uri, signal) {
		...
		return request(this.base(), key, `/api/v1/content/read?${qs.toString()}`, { method: "GET" }, timeoutMs, signal);
	}
	...
	async deleteFile(uri, signal) {
		...
		return request(this.base(), key, `/api/v1/fs?${qs.toString()}`, { method: "DELETE" }, timeoutMs, signal);
	}
};
//#endregion
...
function registerOvTools(ctx, getConfig) {
	...
	ctx.tools.guard((execution) => {
		if (typeof execution.name !== "string") return void 0;
		if (!OV_TOOL_NAMES.includes(execution.name)) return void 0;
		const c = getConfig();
		if (c.url.trim() !== "" && c.key.trim() !== "") return void 0;
...
function apply(ctx, entry) {
	...
	ctx.on("agent/pre-step", async ({ signal }, next) => {
		...
		if (last === void 0 || last.source.kind !== "user") return decision;
